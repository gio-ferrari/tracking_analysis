...
from tracking_analysis.numba_func import _njit, nb_trunc_shift_exp_conv_eval_fullfs, calc_delta_t, calc_cost
...
class BasicFit():
    ...
    def example_conv(self, tau, shift, a1, a2, a3):
        ...
        for pulse_idx in range(NUM_PULSES):
            convoluted_data += weights[pulse_idx]*nb_trunc_shift_exp_conv_eval_fullfs(
                self.nanot_ax_ns,
    ...
    def _calc_monoexp(self, tau: np.float64, shift: np.float64, c_bg: np.float64) -> np.float64:
        """
        ...
        self.proc_data.bins_microtime,
        self.cut_normed_vals_IRF,
        tau,
        shift,
        c_bg,
        self.fit_params.bin_microtime_ns,
        self.proc_data.num_photons,
        self.out_len,
        self.n_fft
        )
        # model_fn_eval *= n_phot
        res = calc_cost(model_fn_eval, self.normed_cnts)
        return res
        
    def _eval_minuit_fit_monoexp(
        self,
        fit_params_w_guess: dict[str, float],
        fit_params_w_lims: dict[str, tuple[float | None, float | None]]
    ) -> tuple[float | None, dict[str, float], bool]:
        """
        Run the fit with Minuit
        """
        minuit = Minuit(self._calc_monoexp, grad=None, **fit_params_w_guess)
        minuit.errordef = Minuit.LIKELIHOOD
        for param, lims in fit_params_w_lims.items():
            minuit.limits[param] = lims
        minuit.migrad()
